...
export class xmlCreator {
	...
	goToLastInsertedPara() {
		...
		if (
			this.currentElement.content.at(-1) &&
			this.currentElement.content.at(-1).name !== 'listItem'
		) {
			this.currentElement = this.currentElement.content.at(-1)
		} else if (
			this.currentElement.content.at(-1) &&
			this.currentElement.content.at(-1).content.at(-1)
		) {
	...
	chooseListVariant(paragraf) {
		if (
			paragraf.endsWith(':') &&
			this.currentElement.name === 'randomList' &&
			this.currentElement.content[0]
		) {
	...
	isTableNew(element = this.currentElement) {
		if (
			element.content.at(-1).name === 'para' &&
			element.content
				.at(-1)
				.content[0].openTag.startsWith('Продолжение таблицы')
		) {
			element.content.pop()
			// console.log('new')///////////////////////////
			...
			return element.content
				.at(-1)
				.content[1].content.find(el => el.name === 'tbody')
		}
		return null
	}
		...
		if (element.content.at(-1).name === 'para') {
			...
			if (paraString.startsWith('Таблица')) {
				...
				for (let i = 1; i < titleArr.length; i++) {
					if (id) {
						if (titleArr[i] === '–' || titleArr[i] === '-') continue
						titleText.length
							? (titleText = titleText + ' ' + titleArr[i])
							: (titleText = titleText + titleArr[i])
		...
		for (let i = 0; i < tableInfo.globalrows.length; i++) {
			...
			for (let j = 0; j < tableInfo.globalrows[i].columns.length; j++) {
				...
				const attributesKeys = Object.keys(
					tableInfo.globalrows[i].columns[j].attributes
				)
				...
				for (let key of attributesKeys) {
					if (key === 'colSpannig') {
						...
						cellNewEndId =
							cellEndId +
							Number(tableInfo.globalrows[i].columns[j].attributes[key])
				...
				if (isMerged) {
					...
					if (this.tableMergeState[j].entryNode) {
						// console.log('merged', this.tableMergeState[j].cellsMerged)
						this.tableMergeState[j].entryNode.addAttribute(
							`morerows="${this.tableMergeState[j].cellsMerged}"`
						)
					}
					//console.log(this.tableMergeState)
				...
				for (
					let k = 0;
					k < tableInfo.globalrows[i].columns[j].paragraphs.length;
					k++
				) {
					...
					let newText = new tags.text(
						tableInfo.globalrows[i].columns[j].paragraphs[k].text
					)
	...
	actualizeSeqStack(paragraf, seqId) {
		...
		if (
			this.levelStack.indexOf(seqId) !== -1 &&
			this.levelStack.at(-1) !== seqId
		) {
			while (this.levelStack.at(-1) !== seqId) {
				if (this.levelStack.at(-1) === null) {
					...
					this.levelStack.at(-1) === 'preserve' &&
					this.currentElement.name === 'listItem'
				) {
			...
			if (
				seqId === 'preserve' &&
				this.seqVariants.indexOf(this.currentElement.name) !== -1
			) {
				this.goDown()
				this.goDown()
				this.goUp()
	...
	chooseTextParagraf(paragraf, seqId = null, bookmarkIds = null) {
		...
		if (this.seqVariants.indexOf(this.currentElement.name) !== -1) {
			...
			this.currentElement.name === 'caution' &&
			paragraf === paragraf.toUpperCase()
		) {
