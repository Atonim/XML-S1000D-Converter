...
export class dmodule extends Tag {
	constructor(infoCode = '018', techName = 'default name') {
		...
		let infoName = infoTable.find(
			element => element.infoCode === infoCode
		).infoName
		this.media = []
		this.name = 'dmodule'
		this.openTag = `!--Arbortext, Inc., 1988-2017, v.4002-->
<?Pub Inc?>
...
export class title extends Tag {
	...
	stringify(level) {
		return `\n${paddng(level)}<${this.openTag}${
			this.attribute
		}>${this.content[0].stringify(0)}<${this.closeTag}>`
	}
}
...
const infoTable = [
	...
	{
		infoCode: '122',
		infoName: 'Указания по включению и опробованию работы изделия',
	},
	{
		infoCode: '123',
		infoName: 'Установка и настройка программного обеспечения',
	},
	{
		infoCode: '410',
		infoName:
			'Перечень возможных неисправностей в процессе использования изделия и рекомендации по действиям при их возникновении',
	},
]
